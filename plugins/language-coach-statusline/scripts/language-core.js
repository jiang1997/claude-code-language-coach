...
async function runLanguageCoach(options = {}) {
  ...
  checkNodeVersion(emitSystemMessage);

  let context = {};
  try {
    await main(options, emitSystemMessage, (nextContext) => {
      context = nextContext || {};
    });
  } catch (error) {
    emitSystemMessage(`Language Coach hook failed: ${formatError(error)}`, context);
  }
}

async function main(options, emitSystemMessage, setContext) {
  const input = parseHookInput(await readStdin());
  if (input.hook_event_name !== "UserPromptSubmit") {
    return;
  }

  const context = typeof options.createContext === "function" ? options.createContext(input) || {} : {};
  setContext(context);
...
function checkNodeVersion(emitSystemMessage) {
